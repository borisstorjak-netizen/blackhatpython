...
def request_handler(buffer):
    # performes packet modifications
    return buffer
...
def proxy_handler(client_socket, remot_host, remote_port, receive_first):
    ...
    while True:
        ...
        if len(local_buffer):
            linecache = "[==>]Recived %d bytes from localhost." % len(remote_buffer)
            print(line)
...
def server_loop(local_host, local_port,
                remot_host, remot_port, recive_first):
    ...
    try:
        ...
        print("[!!] Check for other listening sockets or correct permisons.")
        sys.exit(0)
    ...
    while True:
        ...
        proxy_thread = threading.Thread(
            ...
            args=(client_socket, remot_host, remot_port, recive_first))
        proxy_thread.start()
